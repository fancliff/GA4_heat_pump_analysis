...
while True:
    try:
        Twmax = float(input('Enter system max water temperature in Celsisus: '))
        if Twmax < 30 or  Twmax > 90:
            print("Maximum water temperature must be between 30 and 90 degC")
        else:
            break
    except ValueError:
        print("Please enter a number only.")
print('')
Twmax += 273.15 #Convert to K
...
T2 = Twmax + 10 # allow for adequate temperature difference for heat transfer
...
def plot_Ts(resolution):
    ...
    plt.savefig('Transcrit_T_s_30.eps', format = 'eps')
    plt.show()
...
def plot_Ph(resolution):
    ...
    #plt.grid(which="both")
    plt.legend(loc = 'upper left')
    plt.savefig('Transcrit_P_h_30.eps', format = 'eps')
    plt.show()
